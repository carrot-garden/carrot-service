...
# user name file
WRAPPER_CONF_USER="./conf/wrapper.account.properties"
#
...
#
# NOTE:
#	this value bust be overriten from wrapper.account.properties file
#
RUN_AS_USER="MUST_OVERRIDE_THIS"
...
# make absolute WRAPPER_CONF_USER
FIRST_CHAR=`echo $WRAPPER_CONF_USER | cut -c1,1`
if [ "$FIRST_CHAR" != "/" ] ; then
    WRAPPER_CONF_USER=$REAL_PATH/$WRAPPER_CONF_USER
fi
...
checkUserExists(){
	...
	# log "try to use alternative user account configuration source"

	if [ -r "$WRAPPER_CONF_USER" ]; then
	    # log "file exists and readable: $WRAPPER_CONF_USER"
	    USER_NAME_PROP="wrapper.uxservice.account"
	    # delete lines starting with "#"; find lines containing property name; use last line; trim key name and spaces;
	    USER_NAME=`sed '/^\#/d' "$WRAPPER_CONF_USER" | grep "$USER_NAME_PROP"  | tail -n 1 | sed 's/^.*=//;s/^[[:space:]]*//;s/[[:space:]]*$//'`
	    if [ "X$USER_NAME" != "X" ] ; then
		RUN_AS_USER=$USER_NAME
		# log "set alternative user:  $RUN_AS_USER"
	    else
		: #
		# log "USER_NAME is empty"
	    fi
	else
	    : #
	    # log "not found: $WRAPPER_CONF_USER"
	fi
